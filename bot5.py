...
async def select_mode(update: Update, context):
    ...
    if query.data == "mode_user":
        ...
        if VIDEO_LIBRARY:
            ...
            await query.edit_message_text("User mode selected.\nPlease choose a video category:",
                                          reply_markup=reply_markup)
        else:
            await query.edit_message_text("User mode selected, but no videos are available. Type '/end_session' to "
                                          "exit from user mode")
...
async def admin_password(update: Update, context):
    ...
    if password == "hostertoster":
        ...
        await update.message.reply_text("Incorrect password. Please try again or type '/end_session' to exit from "
                                        "admin mode: ")
        return ADMIN_PASSWORD
